...
def collectArroundCliq(bn, jt, ca, index, diffu):
    ...
    for i in neigh:
        if([i,ca] in diffu[:index-1]): #The potential is a "diffusion's potential"1
...
def inference(bn, jt, absorp, diffu, targets, targetmp): 
    ...
    if(diffu):
        for i in range(len(diffu)):
            sendMessDiffu(bn, jt, diffu[i][0], diffu[i][1], i, diffu)
    return diffu
